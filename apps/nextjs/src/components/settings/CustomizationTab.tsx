  ...
  const fetchProfile = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("user_profiles")
        .select(
          "preferred_name, occupation, chat_traits, selected_theme, custom_primary_color",
        )
        .eq("user_id", user.id)
        .maybeSingle();
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            {themeOptions.map((theme) => (
              ...
              >
                <div className="mb-3 flex h-12 w-full rounded">
                  <div
            ...
            >
              <div className="mb-3 flex h-12 w-full items-center justify-center rounded border-2 border-dashed border-gray-300">
                <PaletteIcon className="h-6 w-6 text-gray-400" />
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select traits that describe how you'd like the AI to communicate
            with you
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex min-h-[40px] flex-wrap gap-2 rounded-md border p-3">
            ...
            <div className="flex flex-wrap gap-2">
              {suggestedTraits.map((trait) => (
                <Badge
                  key={trait}
                  variant={
                    profile.chat_traits.includes(trait) ? "default" : "outline"
                  }
                  className="cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700"
                  onClick={() =>
                    profile.chat_traits.includes(trait)
                      ? removeTrait(trait)
                      : addTrait(trait)
                  }
                >
