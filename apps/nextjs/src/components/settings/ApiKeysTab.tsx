...
import { ExternalLinkIcon, EyeIcon, EyeOffIcon, KeyIcon } from "lucide-react";
...
const ApiKeysTab = () => {
  ...
  const fetchApiKeys = async () => {
    ...
    try {
      // Fetch all API keys from user_profiles table
        ...
        const { error } = await supabase.from("user_api_keys").upsert(
          {
            user_id: user.id,
            provider: "openrouter",
            api_key_encrypted: keyValue.trim() || "",
          },
          {
  ...
  return (
    <div className="space-y-6">
      <Alert>
        <AlertDescription>
          <strong>Smart API Routing:</strong> When you add your own API keys,
          they will be used directly for better performance and lower costs.
          OpenRouter is used as a fallback when direct API keys are not
          available.
        </AlertDescription>
      </Alert>
