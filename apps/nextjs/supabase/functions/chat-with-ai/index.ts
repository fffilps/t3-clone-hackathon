...
const SYSTEM_PROMPT = "You are a professional, of all sorts. Every response, you must first think of who is the best at this task and then place yourself as that role, and then you are a coach and an assistant. Your tone is relaxed like a skateboarder but still professional.";
...
const callAnthropic = async (messages: any[], model: string, apiKey: string) => {
...
const callGoogle = async (messages: any[], model: string, apiKey: string) => {
  ...
  const messagesWithSystem = [
    ...
    { role: 'model', content: 'Understood. I will think of who is best at each task, assume that role as a coach and assistant, and maintain a relaxed but professional tone like a skateboarder.' },
    ...messages.filter(m => m.role !== 'system')
  ];
...
const callOpenRouter = async (messages: any[], model: string, apiKey: string) => {
  ...
  try {
    ...
    try {
      if (route.useDirectApi && route.apiKey) {
        ...
        switch (route.provider) {
          case 'openai':
            ...
            break;
          case 'anthropic':
            assistantMessage = await callAnthropic(messages, model, route.apiKey);
            break;
          case 'google':
        ...
        throw new Error('No direct API key available, using OpenRouter fallback');
      }
      ...
      if (openRouterError || !openRouterKeyData) {
        throw new Error(`Direct API failed: ${directApiError.message}. No OpenRouter fallback available. Please add your API keys in settings.`);
      }

      console.log('Falling back to OpenRouter after direct API failure');
      assistantMessage = await callOpenRouter(messages, model, openRouterKeyData.api_key_encrypted);
