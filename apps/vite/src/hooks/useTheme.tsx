  ...
  useEffect(() => {
    ...
    const fetchTheme = async () => {
      ...
      if (data?.selected_theme) {
        ...
        applyTheme(data.selected_theme);
      } else {
  ...
  const applyTheme = (selectedTheme: string) => {
    ...
    if (themeConfig) {
      ...
      Object.entries(colors).forEach(([key, value]) => {
        ...
        root.style.setProperty(`--${key}`, `${hslColor.h} ${hslColor.s}% ${hslColor.l}%`);
      });
      
      // Set foreground colors
      root.style.setProperty('--primary-foreground', isDark ? '0 0% 100%' : '0 0% 0%');
      root.style.setProperty('--secondary-foreground', isDark ? '0 0% 100%' : '0 0% 0%');
      root.style.setProperty('--accent-foreground', isDark ? '0 0% 100%' : '0 0% 0%');
      root.style.setProperty('--muted-foreground', isDark ? '0 0% 70%' : '0 0% 40%');
